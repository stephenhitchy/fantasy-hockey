...
**Batch:** B1C  
**Runtime release being frozen:** Release Candidate 23  
...
B1C is repository and release-operations tooling. The B1C tooling itself does not deploy or mutate production. This maintained runbook now targets the current Release Candidate 23 runtime after S3E is deployed; Scoring V3 and Projection V11 remain unchanged.
...
Do not run `npm run build:all` merely to deploy B1C. The exact competitive runtime remains the already deployed Release Candidate 23 build.
...
- The live domain serves Release Candidate 23, Scoring V3, and Projection V11.
...
- The runtime release label remains RC23.

## Produce the exact-build validation JSON

On the deployed Release Candidate 23 Release Readiness page:
...
```bash
pbpaste > "$HOME/Downloads/rinkrat-rc23-validation.json"
```
...
node -e "JSON.parse(require('fs').readFileSync(process.argv[1], 'utf8')); console.log('Validation JSON is readable.');" \
  "$HOME/Downloads/rinkrat-rc23-validation.json"
```
...
npm run beta:freeze -- \
  --validation-report="$HOME/Downloads/rinkrat-rc23-validation.json" \
  --tag=rinkrat-rc23-invite-beta \
  --ci-passed \
  --rollback-rehearsed \
  --queue-shadow
```
...
Review the generated JSON and rollback Markdown, then create the annotated tag exactly as printed by the command. The tag deliberately points to the source revision recorded in the live RC23 manifest, not automatically to the newer B1C tooling commit.

Example:

```bash
git tag -a rinkrat-rc23-invite-beta LIVE_SOURCE_REVISION \
  -m "RinkRat RC23 invite beta baseline"
git push origin rinkrat-rc23-invite-beta
```

Verify the tag:

```bash
npm run beta:verify-tag -- --tag=rinkrat-rc23-invite-beta
```

Verify the complete frozen state while RC23 remains live:

```bash
npm run beta:verify-freeze -- --tag=rinkrat-rc23-invite-beta
```

Regenerate the rollback plan later without changing the record:

```bash
npm run beta:rollback-plan -- --tag=rinkrat-rc23-invite-beta
```
