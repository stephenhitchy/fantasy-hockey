...
**Batch:** B1C
**Runtime release being frozen:** Release Candidate 56
...
B1C remains the repository and release-operations tooling, and the tooling itself does not deploy or mutate production. This maintained runbook now targets the current Release Candidate 56 / Operations Batch O1F runtime; Production Scoring V4 and Projection V11 are the frozen competition models after the guarded preseason migration.
...
Commit and push the verified RC56 source:
...
Perform the cutover during a quiet maintenance window. Deploy the complete V4-aware Functions source and make the verified RC56 browser available before operating a V4 league. RC56 is dual-version-aware: historical V3 leagues continue displaying and scoring as V3 until an explicit migration, while eligible/new V4 leagues use V4. Do not create, schedule, or start a Draft during the brief Functions/Hosting transition.
...
Regenerate that league's Projection V11 snapshot from the verified RC56 interface, then inspect it:
...
- The live domain serves Release Candidate 56, Scoring V4, and Projection V11.
...
- The runtime release label remains RC56.

## Produce the exact-build validation JSON

On the deployed Release Candidate 56 Release Readiness page:
...
```bash
pbpaste > "$HOME/Downloads/rinkrat-rc56-validation.json"
```
...
node -e "JSON.parse(require('fs').readFileSync(process.argv[1], 'utf8')); console.log('Validation JSON is readable.');" \
  "$HOME/Downloads/rinkrat-rc56-validation.json"
```
...
4. Review the RC56 rollback selectors: Firestore Rules, complete Functions, and Hosting from the same known-good revision.
...
npm run beta:freeze -- \
  --validation-report="$HOME/Downloads/rinkrat-rc56-validation.json" \
  --tag=rinkrat-rc56-invite-beta \
  --ci-passed \
  --rollback-rehearsed \
  --queue-shadow
```
...
Review the generated JSON and rollback Markdown, then create the annotated tag exactly as printed by the command. The tag deliberately points to the source revision recorded in the live RC56 manifest, not automatically to a newer release-tooling commit.

Example:

```bash
git tag -a rinkrat-rc56-invite-beta LIVE_SOURCE_REVISION \
  -m "RinkRat RC56 invite beta baseline"
git push origin rinkrat-rc56-invite-beta
```

Verify the tag:

```bash
npm run beta:verify-tag -- --tag=rinkrat-rc56-invite-beta
```

Verify the complete frozen state while RC56 remains live:

```bash
npm run beta:verify-freeze -- --tag=rinkrat-rc56-invite-beta
```

Regenerate the rollback plan later without changing the record:

```bash
npm run beta:rollback-plan -- --tag=rinkrat-rc56-invite-beta
```
