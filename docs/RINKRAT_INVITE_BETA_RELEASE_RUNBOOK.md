...
**Batch:** B1C
**Runtime release being frozen:** Release Candidate 57
...
B1C remains the repository and release-operations tooling, and the tooling itself does not deploy or mutate production. This maintained runbook now targets the current Release Candidate 57 / Operations Batch O1G runtime; Production Scoring V4 and Projection V11 are the frozen competition models after the guarded preseason migration.
...
O1F adds the signed-in Privacy Center, immediate bounded JSON export, privacy-request operations, scheduled cleanup, and account-deletion pseudonymization. It also updates all maintained private-season and incident Functions to require the exact deployed RC57 build. The operational workflow remains subject to jurisdiction-specific legal review before public beta.
...
Commit and push the verified RC57 source:
...
Perform the cutover during a quiet maintenance window. Deploy the complete V4-aware Functions source and make the verified RC57 browser available before operating a V4 league. RC57 is dual-version-aware: historical V3 leagues continue displaying and scoring as V3 until an explicit migration, while eligible/new V4 leagues use V4. Do not create, schedule, or start a Draft during the brief Functions/Hosting transition.
...
Regenerate that league's Projection V11 snapshot from the verified RC57 interface, then inspect it:
...
- The live domain serves Release Candidate 57, Scoring V4, and Projection V11.
...
- The runtime release label remains RC57.

## Produce the exact-build validation JSON

On the deployed Release Candidate 57 Release Readiness page:
...
```bash
pbpaste > "$HOME/Downloads/rinkrat-rc57-validation.json"
```
...
node -e "JSON.parse(require('fs').readFileSync(process.argv[1], 'utf8')); console.log('Validation JSON is readable.');" \
  "$HOME/Downloads/rinkrat-rc57-validation.json"
```
...
4. Review the RC57 rollback selectors: Firestore Rules, complete Functions, and Hosting from the same known-good revision.
...
npm run beta:freeze -- \
  --validation-report="$HOME/Downloads/rinkrat-rc57-validation.json" \
  --tag=rinkrat-rc57-invite-beta \
  --ci-passed \
  --rollback-rehearsed \
  --queue-shadow
```
...
Review the generated JSON and rollback Markdown, then create the annotated tag exactly as printed by the command. The tag deliberately points to the source revision recorded in the live RC57 manifest, not automatically to a newer release-tooling commit.

Example:

```bash
git tag -a rinkrat-rc57-invite-beta LIVE_SOURCE_REVISION \
  -m "RinkRat RC57 invite beta baseline"
git push origin rinkrat-rc57-invite-beta
```

Verify the tag:

```bash
npm run beta:verify-tag -- --tag=rinkrat-rc57-invite-beta
```

Verify the complete frozen state while RC57 remains live:

```bash
npm run beta:verify-freeze -- --tag=rinkrat-rc57-invite-beta
```

Regenerate the rollback plan later without changing the record:

```bash
npm run beta:rollback-plan -- --tag=rinkrat-rc57-invite-beta
```
