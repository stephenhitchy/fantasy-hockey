...
monitor and replaces the Available Players route's fixed four-listener assumption. Authenticated
D1N-B measurement then corrected a second source-audit blind spot: the waiver API opens two streams,
and the shared plus commissioner player-availability streams remain active in league context. The
measured formula is therefore `9 + managersPerLeague + assumedActiveCycles`. The current
one-active-cycle assumption produces 20 listeners for a ten-manager league and 22 for a
twelve-manager league.
...
profiles remain planning inputs even after local non-production sampling. See
...
- **Draft task throughput and fallback coverage still need staged ramps.** Six exact stage-100 runs preserved correctness while proving that concurrency ceilings alone do not prevent Cloud Tasks from smoothing synchronized starts. FF1.39 then passed stage 100 at exact `a462006d` by reserving the authoritative start at T-5 under a bounded sixty-dispatch ceiling. Later client-only releases advanced the exact source revision, so the same-revision rule requires one fresh finalized stage-100 pass before 500.
