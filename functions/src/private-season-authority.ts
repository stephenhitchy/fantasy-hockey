...
const CURRENT_BUILD_ID_PATTERN = /^release-candidate-52-[A-Za-z0-9._:-]{4,160}$/;
...
function buildIdentity(value: unknown, requireDeployableBuild = false): PrivateSeasonBuildIdentity {
  ...
  if (
    ...
    throw new HttpsError(
      'failed-precondition',
      'Refresh RinkRat. The private-season gate accepts only the current RC52 / Scoring V4 / Projection V11 build.',
    );
  }
  if (requireDeployableBuild && result.buildId.endsWith('-local')) {
    throw new HttpsError(
      'failed-precondition',
      'Open the deployed RC52 site before changing or approving the private-season plan.',
    );
  }
  return result;
}
