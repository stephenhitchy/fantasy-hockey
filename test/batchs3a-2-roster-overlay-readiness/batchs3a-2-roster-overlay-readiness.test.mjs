...
test('S3A.2 verification, documentation, release label, and permanent roadmap stay synchronized', () => {
  ...
  assert.match(readmeSource, /Release Candidate \d+ \/ (?:Security S3A\.2|Security S3B(?:\.\d+)?|Onboarding Batch B1A|Security Batch S3C|Security Batch S3D|Security Batch S3E|Beta Operations Batch B1B)/);
