...
function snapshot(overrides = {}) {
  return {
    ...
    sourceReleaseLabel: 'Release Candidate 55',
...
test('documentation and roadmap record exact-route stale access and site-first proof', async () => {
  ...
  assert.match(releaseRunbook, /Release Candidate 55 \/ Operations Batch O1E/);
  assert.match(releaseRunbook, /npm run verify:batcho1e/);
  assert.match(releaseRunbook, /rinkrat-rc55-validation\.json/);
  assert.match(releaseRunbook, /rinkrat-rc55-invite-beta/);
});
