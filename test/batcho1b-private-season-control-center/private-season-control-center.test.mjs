...
function buildIdentity() {
  return {
    ...
    buildId: 'release-candidate-52-test-build',
...
function readyPlan() {
  ...
  plan.freeze = {
    ...
    approvedReleaseLabel: 'Release Candidate 52',
    approvedBuildId: 'release-candidate-52-test-build',
...
test('empty plan fails closed on cohort, coverage, exact-build, and support gates', () => {
  ...
  assert.ok(readiness.blockers.some((item) => item.includes('Release Candidate 52')));
});
...
test('an approved decision is current only for the exact plan revision and build', () => {
  ...
  plan.latestDecision = {
    ...
    planRevision: plan.revision, planHash: hash, releaseLabel: 'Release Candidate 52', buildId: 'release-candidate-52-test-build', recordedAt: '2026-09-27T12:00:00Z', recordedBy: 'admin',
  };
