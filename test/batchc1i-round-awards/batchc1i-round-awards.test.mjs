...
test('League Wire renders compact Pickup and Biggest Upset lines without another listener', async () => {
  ...
  assert.match(template, /Pickup of the Round/);
...
test('C1I advances to RC35 while preserving competitive models, Rules, indexes, and safety modes', async () => {
  const [
    scoringRules,
    scoringEngine,
    projectionV11,
  ...
  assert.equal(freeze.defaultTag, 'rinkrat-rc35-invite-beta');
  ...
  assert.match(packageJson.scripts['verify:batchc1i:core'], /verify:batchc1h:core/);
...
test('C1I documentation and roadmap complete the bounded recap and site-first workflow', async () => {
  ...
  assert.match(releaseRunbook, /npm run verify:batchc1i/);
