...
test('S2B.1 verification, documentation, and permanent roadmap remain synchronized', () => {
  ...
  assert.match(readmeSource, /Release Candidate \d+ \/ (?:S2B\.1 hotfix|Security S3A(?:\.\d+)?|Security S3B(?:\.\d+)?|Onboarding Batch B1A|Security Batch S3C|Security Batch S3D|Security Batch S3E|Security Batch S3F|Beta Operations Batch B1B)/);
