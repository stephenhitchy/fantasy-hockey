...
test('member-only rules and the compact client listener keep League Wire bounded', async () => {
  ...
  assert.doesNotMatch(template, /pending claims, queued plans, invite codes, and admin details stay off the wire/i);
...
test('C1A documentation and permanent roadmap record the bounded foundation and privacy follow-up', async () => {
  ...
  assert.match(freezeRunbook, /rinkrat-rc46-validation\.json/);
  assert.match(freezeRunbook, /rinkrat-rc46-invite-beta/);
});
