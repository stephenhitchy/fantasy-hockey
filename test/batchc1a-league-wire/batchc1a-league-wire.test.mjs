...
test('C1A remains intact under RC30 while preserving competitive models and safety modes', async () => {
  const [
    runtime,
    productionRuntime,
    scoring,
  ...
  assert.equal(freeze.releaseLabel, 'Release Candidate 30');
  assert.equal(freeze.verificationCommand, 'npm run verify:batchc1d');
  ...
  assert.match(packageJson.scripts['security:ci'], /verify:batchc1d:core/);
});
...
test('legacy release and Functions-tree guards recognize the isolated C1A through C1D additions', async () => {
  const executableTests = await listExecutableTests();
  ...
  for (const relativePath of executableTests) {
    ...
    if (
      ...
      !source.includes('Social Batch C1D')
    ) {
...
test('C1A documentation and permanent roadmap record the bounded foundation and privacy follow-up', async () => {
  ...
  assert.match(roadmap, /Version 1\.21/);
  ...
  assert.match(readme, /Release Candidate 30 \/ Social Batch C1D/);
  assert.match(readme, /RINKRAT_SOCIAL_C1A_LEAGUE_WIRE\.md/);
  assert.match(freezeRunbook, /rinkrat-rc30-validation\.json/);
  assert.match(freezeRunbook, /rinkrat-rc30-invite-beta/);
});
