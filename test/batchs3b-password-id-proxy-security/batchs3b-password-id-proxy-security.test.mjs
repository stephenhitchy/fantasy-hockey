...
test('user-controlled competitive path fragments use the shared validator', () => {
  for (const [label, source] of [
    ...
    assert.match(
      source,
      /requireFirestoreDocumentId|isSafeFirestoreDocumentId|optionalFirestoreDocumentId|requireFirestoreDocumentIds/,
...
test('S3B release, documentation, roadmap, and verification commands remain synchronized', () => {
  ...
  assert.match(readmeSource, /Release Candidate 17 \/ Security S3B/);
  ...
  assert.match(roadmapRootSource, /Version 1\.7/);
  assert.match(roadmapRootSource, /# \[x\] S3\.11/);
  assert.match(roadmapRootSource, /# \[x\] S3\.13/);
  assert.match(roadmapRootSource, /\[~\] SEQ\.7/);
  assert.match(runtimeConfigSource, /Release Candidate 17/);
  assert.match(productionRuntimeConfigSource, /Release Candidate 17/);
});
