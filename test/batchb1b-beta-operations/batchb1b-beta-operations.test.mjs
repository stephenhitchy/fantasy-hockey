...
test('B1B evidence collections have source-controlled TTL and scheduled cleanup fallback', async () => {
  ...
  assert.equal(ttlConfig.policies.length, 10);
  assert.equal(retentionConfig.collections.length, 12);
  ...
  assert.ok(retentionConfig.collections.some((item) => item.collection === 'privacyRequestOperations'));
  assert.ok(retentionConfig.collections.some((item) => item.collection === 'privacyExportAudits'));
});
...
test('B1B release, verification, documentation, and permanent roadmap remain synchronized', async () => {
  ...
  assert.match(packageJson.scripts['security:ci'], /verify:batch(?:b1b|b1b-1|b1c|s4a|b1d|s3d|s3e|s3e-1|s3e-1-1|s3f|d1a|d1a-1|d1b|d1c|c1a|c1b|c1c|c1d|c1e|c1f|c1g|c1h|c1i|c1j|c1k|c1l|c1m|a1a|a1b|a1c|a1d|a1e|a1f|a1g|a1h|a1i|n1a|n1b|v4a|o1a|o1b|o1c|o1d|o1e|o1f|o1g|o1h|o1i|b1e|b1f|b1g|b1h|b1i|b1j|d1j):core/);
