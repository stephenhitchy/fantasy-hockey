...
function liveManifest() {
  return {
    schemaVersion: 1,
    releaseLabel: 'Release Candidate 29',
    buildId: 'release-candidate-29-test-aabbccddeeff',
...
test('the source-only preflight validates RC29 controls without requiring the host machine toolchain', () => {
  ...
  assert.match(output, /(?:RC29|Release Candidate 29) runtime/);
});
...
test('freeze and rollback tooling is explicit, non-deploying, and does not create tags silently', async () => {
  ...
  assert.equal(policy.releaseLabel, 'Release Candidate 29');
  ...
  assert.equal(safeTagName('rinkrat-rc29-invite-beta'), 'rinkrat-rc29-invite-beta');
...
test('generated rollback guidance uses the frozen source and the smallest normal rollback', () => {
  const plan = formatRollbackPlan({
    ...
    git: { tag: 'rinkrat-rc29-invite-beta' },
  });

  assert.match(plan, /git checkout rinkrat-rc29-invite-beta/);
...
test('B1C scripts, documentation, roadmap, and CI verification remain synchronized', async () => {
  ...
  assert.match(packageJson.scripts['security:ci'], /verify:batch(?:b1c|s4a|b1d|s3d|s3e|s3e-1|s3e-1-1|s3f|d1a|d1a-1|d1b|d1c|c1a|c1b|c1c):core/);
  ...
  assert.match(runbook, /pbpaste > .*rinkrat-rc29-validation\.json/);
