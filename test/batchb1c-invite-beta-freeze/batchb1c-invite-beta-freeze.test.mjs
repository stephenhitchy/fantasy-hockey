...
test('the source-only preflight validates RC34 controls without requiring the host machine toolchain', () => {
  ...
  assert.match(output, /(?:RC32|Release Candidate 56) runtime/);
});
...
test('freeze and rollback tooling is explicit, non-deploying, and does not create tags silently', async () => {
  ...
  assert.equal(policy.releaseLabel, 'Release Candidate 56');
  ...
  assert.equal(safeTagName('rinkrat-rc56-invite-beta'), 'rinkrat-rc56-invite-beta');
...
test('generated rollback guidance uses the frozen source and the smallest normal rollback', () => {
  const plan = formatRollbackPlan({
    ...
    git: { tag: 'rinkrat-rc56-invite-beta' },
  });

  assert.match(plan, /git checkout rinkrat-rc56-invite-beta/);
...
test('B1C scripts, documentation, roadmap, and CI verification remain synchronized', async () => {
  ...
  assert.match(packageJson.scripts['security:ci'], /verify:batch(?:b1c|s4a|b1d|s3d|s3e|s3e-1|s3e-1-1|s3f|d1a|d1a-1|d1b|d1c|c1a|c1b|c1c|c1d|c1e|c1f|c1g|c1h|c1i|c1j|c1k|c1l|c1m|a1a|a1b|a1c|a1d|a1e|a1f|a1g|a1h|a1i|n1a|n1b|v4a|o1a|o1b|o1c|o1d|o1e|o1f):core/);
  ...
  assert.match(runbook, /pbpaste > .*rinkrat-rc56-validation\.json/);
