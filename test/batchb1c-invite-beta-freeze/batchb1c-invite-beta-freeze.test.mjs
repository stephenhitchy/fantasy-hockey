...
function liveManifest() {
  return {
    schemaVersion: 1,
    releaseLabel: 'Release Candidate 25',
    buildId: 'release-candidate-25-test-aabbccddeeff',
...
test('the source-only preflight validates RC25 controls without requiring the host machine toolchain', () => {
  ...
  assert.match(output, /(?:RC25|Release Candidate 25) runtime/);
});
...
test('freeze and rollback tooling is explicit, non-deploying, and does not create tags silently', async () => {
  ...
  assert.equal(policy.releaseLabel, 'Release Candidate 25');
  ...
  assert.equal(safeTagName('rinkrat-rc25-invite-beta'), 'rinkrat-rc25-invite-beta');
...
test('generated rollback guidance uses the frozen source and the smallest normal rollback', () => {
  const plan = formatRollbackPlan({
    ...
    git: { tag: 'rinkrat-rc25-invite-beta' },
  });

  assert.match(plan, /git checkout rinkrat-rc25-invite-beta/);
...
test('B1C scripts, documentation, roadmap, and CI verification remain synchronized', async () => {
  ...
  assert.match(packageJson.scripts['security:ci'], /verify:batch(?:b1c|s4a|b1d|s3d|s3e|s3e-1|s3e-1-1|s3f|d1a|d1a-1):core/);
  ...
  assert.match(runbook, /pbpaste > .*rinkrat-rc25-validation\.json/);
