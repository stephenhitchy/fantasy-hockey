...
function readyReport(overrides = {}) {
  ...
  return {
    ...
    launchGate: {
      ...
      manualPassedCount: 30,
      manualRequiredCount: 30,
...
test('the source-only preflight validates RC34 controls without requiring the host machine toolchain', () => {
  ...
  assert.match(output, /(?:RC32|Release Candidate 65) runtime/);
});
...
test('freeze and rollback tooling is explicit, non-deploying, and does not create tags silently', async () => {
  ...
  assert.equal(policy.releaseLabel, 'Release Candidate 65');
  ...
  assert.equal(safeTagName('rinkrat-rc65-invite-beta'), 'rinkrat-rc65-invite-beta');
...
test('generated rollback guidance uses the frozen source and the smallest normal rollback', () => {
  const plan = formatRollbackPlan({
    ...
    verification: { command: 'npm run verify:batchb1j' },
    firebase: { projectId: 'nhl-fantasy-app-ab673', hostingTarget: 'app' },
    git: { tag: 'rinkrat-rc65-invite-beta' },
  });

  assert.match(plan, /git checkout rinkrat-rc65-invite-beta/);
  assert.match(plan, /npm install -g npm@11\.17\.0/);
  assert.match(plan, /npm run verify:batchb1j/);
...
test('B1C scripts, documentation, roadmap, and CI verification remain synchronized', async () => {
  ...
  assert.match(packageJson.scripts['security:ci'], /verify:batch(?:b1c|s4a|b1d|s3d|s3e|s3e-1|s3e-1-1|s3f|d1a|d1a-1|d1b|d1c|c1a|c1b|c1c|c1d|c1e|c1f|c1g|c1h|c1i|c1j|c1k|c1l|c1m|a1a|a1b|a1c|a1d|a1e|a1f|a1g|a1h|a1i|n1a|n1b|v4a|o1a|o1b|o1c|o1d|o1e|o1f|o1g|o1h|o1i|b1e|b1f|b1g|b1h|b1i|b1j|d1j):core/);
  ...
  assert.match(runbook, /pbpaste > .*rinkrat-rc65-validation\.json/);
