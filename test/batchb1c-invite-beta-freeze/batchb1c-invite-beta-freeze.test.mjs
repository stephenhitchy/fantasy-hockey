...
function liveManifest() {
  return {
    schemaVersion: 1,
    releaseLabel: 'Release Candidate 26',
    buildId: 'release-candidate-26-test-aabbccddeeff',
...
test('the source-only preflight validates RC26 controls without requiring the host machine toolchain', () => {
...
test('freeze and rollback tooling is explicit, non-deploying, and does not create tags silently', async () => {
  ...
  assert.equal(safeTagName('rinkrat-rc26-invite-beta'), 'rinkrat-rc26-invite-beta');
...
test('generated rollback guidance uses the frozen source and the smallest normal rollback', () => {
  const plan = formatRollbackPlan({
    ...
    git: { tag: 'rinkrat-rc26-invite-beta' },
  });

  assert.match(plan, /git checkout rinkrat-rc26-invite-beta/);
...
test('B1C scripts, documentation, roadmap, and CI verification remain synchronized', async () => {
  ...
  assert.match(runbook, /pbpaste > .*rinkrat-rc26-validation\.json/);
