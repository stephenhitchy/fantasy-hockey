...
function liveManifest() {
  return {
    schemaVersion: 1,
    releaseLabel: 'Release Candidate 31',
...
test('the source-only preflight validates RC31 controls without requiring the host machine toolchain', () => {
  ...
  assert.match(output, /(?:RC31|Release Candidate 31) runtime/);
});
...
test('freeze and rollback tooling is explicit, non-deploying, and does not create tags silently', async () => {
  ...
  assert.equal(policy.releaseLabel, 'Release Candidate 31');
  ...
  assert.equal(safeTagName('rinkrat-rc31-invite-beta'), 'rinkrat-rc31-invite-beta');
...
test('generated rollback guidance uses the frozen source and the smallest normal rollback', () => {
  const plan = formatRollbackPlan({
    ...
    git: { tag: 'rinkrat-rc31-invite-beta' },
  });

  assert.match(plan, /git checkout rinkrat-rc31-invite-beta/);
...
test('B1C scripts, documentation, roadmap, and CI verification remain synchronized', async () => {
  ...
  assert.match(packageJson.scripts['security:ci'], /verify:batch(?:b1c|s4a|b1d|s3d|s3e|s3e-1|s3e-1-1|s3f|d1a|d1a-1|d1b|d1c|c1a|c1b|c1c|c1d|c1e):core/);
  ...
  assert.match(runbook, /pbpaste > .*rinkrat-rc31-validation\.json/);
