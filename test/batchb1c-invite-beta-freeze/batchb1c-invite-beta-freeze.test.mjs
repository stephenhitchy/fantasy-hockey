...
function liveManifest() {
  return {
    schemaVersion: 1,
    releaseLabel: 'Release Candidate 21',
    buildId: 'release-candidate-21-test-aabbccddeeff',
...
test('the source-only preflight validates RC21 controls without requiring the host machine toolchain', () => {
  ...
  assert.match(output, /RC21 runtime/);
});
...
test('freeze and rollback tooling is explicit, non-deploying, and does not create tags silently', async () => {
  ...
  assert.equal(policy.releaseLabel, 'Release Candidate 21');
  ...
  assert.equal(safeTagName('rinkrat-rc21-invite-beta'), 'rinkrat-rc21-invite-beta');
...
test('generated rollback guidance uses the frozen source and the smallest normal rollback', () => {
  const plan = formatRollbackPlan({
    ...
    git: { tag: 'rinkrat-rc21-invite-beta' },
  });

  assert.match(plan, /git checkout rinkrat-rc21-invite-beta/);
...
test('B1C scripts, documentation, roadmap, and CI verification remain synchronized', async () => {
  ...
  assert.match(packageJson.scripts['security:ci'], /test:batchb1b-1:run/);
  assert.match(packageJson.scripts['security:ci'], /test:batchb1c:run/);
  ...
  assert.match(roadmapRoot, /Version 1\.10/);
  assert.match(roadmapRoot, /B1\.24 .*invite-beta freeze/i);
  assert.match(roadmapRoot, /LOG\.18 .*B1C/i);
});

test('B1C preserves the RC21 competitive runtime byte-for-byte', async () => {
  ...
  for (const [relativePath, expectedHash] of Object.entries(preserved)) {
    assert.equal(await hash(relativePath), expectedHash, `${relativePath} changed during repository-only B1C tooling`);
  }
});
