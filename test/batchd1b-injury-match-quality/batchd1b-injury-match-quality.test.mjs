...
test('D1B remains intact under RC30 while preserving Scoring V3, Projection V11, monitor mode, and Shadow', async () => {
  const [
    runtime,
    productionRuntime,
    scoringRules,
  ...
  assert.equal(freezePolicy.releaseLabel, 'Release Candidate 30');
  assert.equal(freezePolicy.verificationCommand, 'npm run verify:batchc1d');
  assert.equal(freezePolicy.defaultTag, 'rinkrat-rc30-invite-beta');
});
