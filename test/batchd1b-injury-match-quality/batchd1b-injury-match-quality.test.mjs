...
test('D1B remains intact under RC34 while preserving Scoring V3, Projection V11, monitor mode, and Shadow', async () => {
  ...
  assert.match(packageJson.scripts['security:ci'], /verify:batch(?:d1b|d1c|c1a|c1b|c1c|c1d|c1e|c1f|c1g|c1h|c1i|c1j|c1k|c1l|c1m|a1a|a1b|a1c|a1d|a1e|a1f|a1g|a1h|a1i|n1a|n1b|v4a|o1a|o1b|o1c):core/);
  assert.equal(freezePolicy.releaseLabel, 'Release Candidate 53');
  assert.equal(freezePolicy.verificationCommand, 'npm run verify:batcho1c');
  assert.equal(freezePolicy.defaultTag, 'rinkrat-rc53-invite-beta');
});
