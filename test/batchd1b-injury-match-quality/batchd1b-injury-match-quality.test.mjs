...
test('D1B remains intact under RC32 while preserving Scoring V3, Projection V11, monitor mode, and Shadow', async () => {
  const [
    runtime,
    productionRuntime,
    scoringRules,
  ...
  assert.match(packageJson.scripts['security:ci'], /verify:batch(?:d1b|d1c|c1a|c1b|c1c|c1d|c1e|c1f):core/);
  assert.equal(freezePolicy.releaseLabel, 'Release Candidate 32');
  assert.equal(freezePolicy.verificationCommand, 'npm run verify:batchc1f');
  assert.equal(freezePolicy.defaultTag, 'rinkrat-rc32-invite-beta');
});
