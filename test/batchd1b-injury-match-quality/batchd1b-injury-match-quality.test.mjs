...
test('D1B advances to RC29 while preserving Scoring V3, Projection V11, monitor mode, and Shadow', async () => {
  const [
    runtime,
    productionRuntime,
    scoringRules,
  ...
  assert.match(packageJson.scripts['security:ci'], /verify:batch(?:d1b|d1c|c1a|c1b|c1c):core/);
  assert.equal(freezePolicy.releaseLabel, 'Release Candidate 29');
  assert.equal(freezePolicy.verificationCommand, 'npm run verify:batchc1c');
  assert.equal(freezePolicy.defaultTag, 'rinkrat-rc29-invite-beta');
});
