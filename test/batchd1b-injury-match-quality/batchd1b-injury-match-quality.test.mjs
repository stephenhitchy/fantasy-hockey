...
test('D1B remains intact under RC34 while preserving Scoring V3, Projection V11, monitor mode, and Shadow', async () => {
  ...
  assert.match(packageJson.scripts['security:ci'], /verify:batch(?:d1b|d1c|c1a|c1b|c1c|c1d|c1e|c1f|c1g|c1h|c1i|c1j|c1k|c1l|c1m|a1a|a1b|a1c|a1d|a1e|a1f|a1g):core/);
  assert.equal(freezePolicy.releaseLabel, 'Release Candidate 45');
  assert.equal(freezePolicy.verificationCommand, 'npm run verify:batcha1g');
  assert.equal(freezePolicy.defaultTag, 'rinkrat-rc45-invite-beta');
});
