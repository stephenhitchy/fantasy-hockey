...
test('D1B remains intact under RC31 while preserving Scoring V3, Projection V11, monitor mode, and Shadow', async () => {
  const [
    runtime,
    productionRuntime,
    scoringRules,
  ...
  assert.equal(freezePolicy.releaseLabel, 'Release Candidate 31');
  assert.equal(freezePolicy.verificationCommand, 'npm run verify:batchc1e');
  assert.equal(freezePolicy.defaultTag, 'rinkrat-rc31-invite-beta');
});
